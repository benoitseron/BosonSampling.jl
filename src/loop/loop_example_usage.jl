...
mode = 1
BeamSplitter(η[mode]).U


LossyBeamSplitter(η[mode], η_loss[mode]).U

virtual_interferometer_uniform_loss(beam_splitter(η[mode]),η_loss[mode])



circuit.U

o = FockDetection(ModeOccupation([2,0]))
ev = Event(i,o, circuit)
@show compute_probability!(ev)
