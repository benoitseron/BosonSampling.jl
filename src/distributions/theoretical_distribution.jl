...
!!! note
    - The probabilities within the distribution are indexed following the same order as `output_mode_occupation(n,m)` which returns an array of all the possible configurations of ``n`` photons among ``m`` modes.
