...
function gram_matrix_from_x(number_photons::Int, x::Real)

	S = 1.0 * Matrix(I, number_photons, number_photons)

	for i = 1:number_photons
		for j = 1:number_photons
...
function beam_splitter(n::Int, reflectivity::Float64)

	B_2 = [sqrt(1-reflectivity) sqrt(reflectivity);
		sqrt(reflectivity) -sqrt(1-reflectivity)]
...
function rotation_matrix(n::Int, angle::Float64)

	R_2 = [cos(angle) -sin(angle);
			sin(angle) cos(angle)]
...
function phase_shift(n::Int, param::Float64)
